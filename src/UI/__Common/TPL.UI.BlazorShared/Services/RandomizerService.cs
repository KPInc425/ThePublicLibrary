namespace TPL.UI.BlazorShared.Services;
public class RandomizerService
{
    ...
    {
        var rnd = new Random().Next(1, 30);
        return $"background-image: url('./itmra/big_background ({rnd}).png'); height: 100vh;background-repeat: no-repeat;background-size: cover;background-attachment: fixed;background-position: center center";
    }
    public string RandomBackgroundStyle(long count)
    {
        ...
        {
            var rnd = new Random().Next(1, 30);
            tempStr += $"url('./itmra/big_background ({rnd}).png'),";
        }
